...
export function sassBuildTask(dest: string, root: string) {
  return () => {
    return gulp.src(resolve(root, '**/*.scss'))
      .pipe(gulpSourcemaps.init())
...
export function copyTask(srcGlobOrDir: string | string[], outRoot: string) {
  if (typeof srcGlobOrDir === 'string') {
    return () => gulp.src(resolve(srcGlobOrDir)).pipe(gulp.dest(outRoot));
  } else {
    return () => gulp.src(srcGlobOrDir.map(name => resolve(name))).pipe(gulp.dest(outRoot));
  }
}
