...
import {COMPONENTS_DIR, DIST_COMPONENTS_ROOT, PROJECT_ROOT, resolve} from '../settings/paths';
import {HTML_MINIFIER_OPTIONS, LICENSE_BANNER, MODULE_BUNDLE, MODULE_NAME } from '../settings/constants';
import {sassBuildTask, tsBuildTask, execNodeTask, copyTask, sequenceTask,triggerLivereload} from '../helpers';
...
const tsconfigPath = path.relative(PROJECT_ROOT, resolve(COMPONENTS_DIR, 'tsconfig.json'));


/** [Watch task] Rebuilds (ESM output) whenever ts, scss, or html sources change. */
task(':watch:components', () => {
  watch(resolve(COMPONENTS_DIR, '**/*.ts'), ['build:components', triggerLivereload]);
  watch(resolve(COMPONENTS_DIR, '**/*.scss'), ['build:components', triggerLivereload]);
  watch(resolve(COMPONENTS_DIR, '**/*.html'), ['build:components', triggerLivereload]);
});
...
task(':build:components:assets', copyTask([
  resolve(COMPONENTS_DIR, '**/*.!(ts|spec.ts)'),
  resolve(PROJECT_ROOT, 'README.md'),
  resolve(PROJECT_ROOT, 'LICENSE'),
...
task(':build:components:rollup', () => {
  ...
  return src(resolve(DIST_COMPONENTS_ROOT, 'index.js'))
    .pipe(gulpRollup(rollupOptions, rollupGenerateOptions))
    .pipe(dest(resolve(DIST_COMPONENTS_ROOT, 'bundles')));
});
