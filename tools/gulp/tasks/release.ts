...
import {DIST_COMPONENTS_ROOT, DIST_ROOT, resolve} from '../settings/paths';


const argv = minimist(process.argv.slice(3));


task(':build:release:clean-spec', cleanTask(resolve(DIST_ROOT, '**/*.spec.*')));
  ...
  return new Promise((resolve, reject) => {
    ...
    childProcess.on('close', (code: number) => {
      if (code == 0) {
        ...
        reject(new Error(`Project did not publish, status: ${code}.`));
      }
    });
  });
}
