...
import {
  SOURCE_ROOT, DIST_ROOT, PROJECT_ROOT, dirs, resolve
} from '../settings/paths';
...
const appDir = resolve(SOURCE_ROOT, 'e2e-app');
const outDir = DIST_ROOT;
const PROTRACTOR_CONFIG_PATH = resolve(dirs.tools.test, 'protractor.conf.js');

task(':watch:e2eapp', () => {
  watch(resolve(appDir, '**/*.ts'), [':build:e2eapp:ts']);
  watch(resolve(appDir, '**/*.html'), [':build:e2eapp:assets']);
});
